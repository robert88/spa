(function( $ ){
	...
	*/
	$.fn.fileReader = function( options ) {
		...
		console.log("file",options,!FileAPIProxy.ready)
	...
	*/
	$.fn.fileReader.defaults = {
		...
		label           : null,
		extensions      : null,
		filereader      : '/pc/public/js/lib/upload/filereader.swf', // The path to the filereader swf file
	...
	*/
	window.FileAPIProxy = {
		...
		onFileReaderEvent: function(evt) {
			if (this.debugMode) console.log('FileReader Event ', evt.type, evt, evt.target in this.readers);
		...
		_handleFlashEvent: function(evt) {
			switch (evt.type) {
				...
				case 'load':
					...
					if( typeof currentOpts.load === "function" ){
						currentOpts.load(this.result.replace(/\s+/g,""),currentOpts.file)
					}
